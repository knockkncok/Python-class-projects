...
def AIprint(pop_up,step_number,solution,stop_thread,finish):
    ...
    if stop_thread.is_set():
        pop_up.window.after(1000,lambda:AIprint(pop_up,step_number,solution,stop_thread,finish))
        return
    if len(solution)==0:
        return
    present(pop_up.numbers,solution.pop())
    pop_up.step_label['text']='Number of steps: '+str(step_number)
    pop_up.window.after(1000,lambda:AIprint(pop_up,step_number+1,solution,stop_thread,finish))
...
def number_puzzle():
  ...
  f1=tk.Frame()
  #message send to thread when one AI pop up is finish
  finish=Event()
